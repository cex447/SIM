...
import { parentCode } from './operations-3.41.0.js';
import { cachedPlatform, cachedPlatformByCirculation, normalizePlatformValue } from './isic-3.41.0.js';
...
async function loadRouteCatalog() {
  if (!routesPromise) {
    routesPromise = loadJson('data/ctc-routes-3.41.0.json', 'ctc-routes-3.41.0.json');
  }
...
async function loadMotionGeometry() {
  if (!motionPromise) {
    motionPromise = loadJson('data/ctc-motion-3.41.0.json', 'ctc-motion-3.41.0.json')
      .then(value => {
...
async function loadStationHitGeometry() {
  if (!stationHitPromise) {
    stationHitPromise = loadJson('data/ctc-stations-3.41.0.json', 'ctc-stations-3.41.0.json')
      .then(value => {
