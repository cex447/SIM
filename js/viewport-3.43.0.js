/*
 * SIM+ Beta 3.43.0 · saneamiento de viewport.
 *
 * PLASTIC, iSIC, LIT, SIV y TORNS:
...
function stickyFilterHeight(view) {
  if (!view || !["view-plastic", "view-torns"].includes(view.id)) return 0;
  const filters = view.querySelector(":scope > .plastic-filters, :scope > .torns-filters");
...
function syncStickyTop(state) {
  const top = stickyFilterHeight(state.view);
  state.stickyTop = top;
...
function ensureVerticalOnly(view) {
  ...
  if ("ResizeObserver" in window) {
    ...
    ro.observe(view);
    const filters = ["view-plastic", "view-torns"].includes(view.id)
      ? view.querySelector(":scope > .plastic-filters, :scope > .torns-filters")
      : null;
