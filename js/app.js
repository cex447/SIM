...
import {
  ...
  syncISICQuery,
  tickISIC
} from "./isic-view.js?v=3.21.0";

import { updateOccupancy } from "./occupancy.js?v=3.21.0";
import { BackgroundAudio } from "./audio.js?v=3.21.0";

import { setupViewports, activateViewport } from "./viewport.js?v=3.21.0";
import { initCTC, enterCTC, leaveCTC, updateCTC, ctcDiagnostic } from "./ctc.js?v=3.21.0";
...
async function loadStaticData() {
  const [configResponse, networkResponse] = await Promise.all([
    fetch("data/config.json?v=3.21.0", { cache: "no-store" }),
    fetch("data/network.json?v=3.21.0", { cache: "no-store" })
  ]);
