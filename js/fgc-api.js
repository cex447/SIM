...
export function decodeCirculation(id){
 const s=String(id||""), tail=s.includes("|")?s.split("|").pop():s;
...
export function decodeUnit(ud){
 const s=String(ud||"");if(!s.startsWith("1f2cc"))return null;
...
function get(r,...names){for(const n of names)if(r[n]!==undefined&&r[n]!==null&&r[n]!=="")return r[n];return null}
export function normalizeTrain(r,cfg){
 const id=String(get(r,"id","trip_id")||""); const ud=String(get(r,"ud","vehicle_id")||"");
 if(!id.startsWith(cfg.idPrefix)||!ud.startsWith(cfg.udPrefix))return null;
 const circulation=decodeCirculation(id),unit=decodeUnit(ud);if(!circulation||!unit)return null;
 const line=String(get(r,"lin","linea","route_short_name")||"");
 const stationed=get(r,"estacionat_a","estacionado_en");
 const next=get(r,"properes_parades","proximas_paradas");
 const origin=get(r,"origen"),dest=get(r,"desti","destino");
 let whereText=stationed?"Est. "+stationed:(next?"En circulació":"En circulació");
 return {raw:r,id,ud,circulation,unit,line,origin,dest,stationed,next,whereText,ascending:Number(circulation.slice(-1))%2===1};
}
