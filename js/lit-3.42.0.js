...
import { getTripBundle } from "./gtfs-3.42.0.js";
import {
  countdownState,
  formatCountdown,
  formatDeparture,
  resolveGtfsMinuteTimestamp
} from "./time-3.42.0.js";
import {
  locateOperationalTarget,
  parentCode
} from "./operations-3.42.0.js";
import {
  cachedPlatform,
  clearPlatform,
  fetchIsicStation,
...
const LIT_COUNTDOWN_RED_SECONDS = 10;
...
export function nextScheduledCountdown(stops, nowMs = Date.now()) {
  for (let index = 0; index < (stops || []).length; index += 1) {
    const stop = stops[index];
    const reference = stop?.departure_time || stop?.arrival_time;
    const targetMs = resolveGtfsMinuteTimestamp(reference, nowMs);
    ...
    return {
      ...
      stop,
      targetMs,
      seconds: Math.max(1, Math.ceil((targetMs - nowMs) / 1000))
    };
  }
...
function updateScheduleCountdown(S, nowMs = Date.now()) {
  const target = nextScheduledCountdown(S.selected?.stops || [], nowMs);
  if (!target) return;

  const item = document.querySelector(`.lit-item[data-i="${target.index}"]`);
  const count = item?.querySelector(".count");
  if (!count) return;

  count.textContent = formatCountdown(target.seconds);
  count.classList.toggle("red", target.seconds <= LIT_COUNTDOWN_RED_SECONDS);
}
...
export function updateCurrent(S, forceScroll = false) {
  ...
  updateScheduleCountdown(S, Date.now());
  ...
  if (item) {
    ...
    item.querySelector(".time")?.classList.toggle("delayed-text", delayed);
