...
import { resolveGtfsTimestamp } from './time-3.43.0.js';
import { parentCode } from './operations-3.43.0.js';
import { cachedPlatform, cachedPlatformByCirculation, normalizePlatformValue } from './isic-3.43.0.js';
...
async function loadRouteCatalog() {
  if (!routesPromise) {
    routesPromise = loadJson('data/ctc-routes-3.43.0.json', 'ctc-routes-3.43.0.json');
  }
...
async function loadMotionGeometry() {
  if (!motionPromise) {
    motionPromise = loadJson('data/ctc-motion-3.43.0.json', 'ctc-motion-3.43.0.json')
      .then(value => {
...
async function loadStationHitGeometry() {
  if (!stationHitPromise) {
    stationHitPromise = loadJson('data/ctc-stations-3.43.0.json', 'ctc-stations-3.43.0.json')
      .then(value => {
...
function setToolbarStationSelection(code = '') {
  selectedToolbarStation = String(code || '').toUpperCase();
  document.querySelectorAll('[data-ctc-focus-station]').forEach(button => {
    const selected = Boolean(selectedToolbarStation) &&
      button.dataset.ctcFocusStation === selectedToolbarStation;
  ...
  document.querySelectorAll('[data-ctc-focus-station]').forEach(button => {
    ...
    button.addEventListener('click', () => {
      ...
      if (applyStationFocus(code, latestState)) {
        ...
        setToolbarStationSelection(code);
      }
    });
  });
