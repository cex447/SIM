import { resolveGtfsTimestamp } from "./time-3.41.0.js";
...
export async function getStationDepartures(
  ...
  const [{ stationRows, tripRows, bounds }, trips, services] = await Promise.all([
  ...
  for (const row of rows) {
    ...
    const orderedStops = tripRows.get(row.trip_id) || [];
    const rowIndex = orderedStops.findIndex(candidate =>
      Number(candidate.stop_sequence) === Number(row.stop_sequence)
    );
    const previous = rowIndex > 0
      ? orderedStops.slice(0, rowIndex).reverse().find(candidate =>
          parentCode(candidate) !== station
        ) || null
      : null;
    const next = rowIndex >= 0
      ? orderedStops.slice(rowIndex + 1).find(candidate =>
          parentCode(candidate) !== station
        ) || null
      : null;
    const tripStops = orderedStops.reduce((codes, candidate) => {
      const code = parentCode(candidate);
    ...
    out.push({
      ...
      previousStop: previous ? {
        code: parentCode(previous),
        name: previous.stop_name || ""
      } : null,
      nextStop: next ? {
