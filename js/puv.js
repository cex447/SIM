...
const LINE_ASSETS = Object.freeze({
  L6: "assets/lines/l6.png",
  S1: "assets/lines/s1.png",
  S2: "assets/lines/s2.png",
  L7: "assets/lines/l7.png",
  L12: "assets/lines/l12.png"
});
...
function syncFilterVisuals(S) {
  document
    ...
    .forEach(button => {
      /*
       * Sin filtro de línea = se ven todas, pero ninguna está "seleccionada":
       * 35 % de opacidad. Al seleccionar una o varias pasan al 100 %.
       */
      const selected = S.puvFilters.lines.has(
  ...
  document
    ...
    .forEach(button => {
      const selected = S.puvFilters.units.has(
...
function stationNode(code) {
  return make("strong", "station-token", code || "—");
}
...
function updateTrainRow(model, train) {
  ...
  model.unit.classList.toggle(
    "delayed-text",
    delayed
  );

  model.circulation.classList.toggle(
    "delayed-text",
    delayed
  );
...
function emptyMessage(S) {
  ...
  if (units.length > 1) {
    return (
      "ACTUALMENT NO CIRCULEN " +
      units.map(value => `UT${value}`).join(" / ")
    );
  }
...
export function renderPUV(S) {
  ...
  const asc = filtered.filter(
    train => train.ascending
  );

  const desc = filtered.filter(
    train => !train.ascending
  );
