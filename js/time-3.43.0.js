...
/* LIT muestra horas con precisión de minuto. Su cuenta atrás debe vencer en
   el mismo instante visible (hh:mm:00), nunca en segundos GTFS ocultos. */
export function resolveGtfsMinuteTimestamp(value, nowMs = Date.now()) {
  const seconds = parseGtfsSeconds(value);
  if (seconds === null) return null;

  const minuteSeconds = Math.floor(seconds / 60) * 60;
  const candidates = [-1, 0, 1].map(dayOffset =>
    localMidnight(nowMs, dayOffset) + minuteSeconds * 1000
  );
