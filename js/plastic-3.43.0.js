...
import { getTripBundle } from "./gtfs-3.43.0.js";
import { occupancyFingerprint, updateOccupancy } from "./occupancy-3.43.0.js";
import { countdownState, formatOperationalCountdown } from "./time-3.43.0.js";
import {
  isOriginHold,
  parentCode
} from "./operations-3.43.0.js";
import {
  cachedPlatform,
  clearPlatform,
  fetchIsicStation,
...
/* Orden único de líneas en todo SIM+: S1, S2, L6, L7, L12. */
const FAMILY_ORDER = Object.freeze(["D", "F", "A", "B", "L"]);
/* Beta 3.43: el carrusel de retrasos de PLASTIC recorre el doble de distancia
   por segundo que en 3.42 (28,8 px/s -> 57,6 px/s). */
const PLASTIC_DELAY_TICKER_SPEED_PX_PER_SECOND = 57.6;
...
function startDelayTicker(refs, key) {
  ...
  requestAnimationFrame(() => {
    ...
    /* Si el text cap íntegrament, es manté estàtic. Si no, es duplica i
       circula exactament a 57,6 px/s. */
...
export function delayedEntries(S) {
...
/* TORNS reutilitza exactament el mateix criteri quantificat que PLASTIC.
   Aquesta funció inicia els contextos horaris que encara falten i permet que
   AFECTACIONS aparegui sense haver d'obrir abans la pantalla PLASTIC. */
export function primeDelayedEntries(S) {
  const missing = (S.trains || []).filter(train => {
    if (train.onTime !== false || !train.id) return false;
    const state = tripContextCache.get(train.id)?.state;
    return state !== "ready" && state !== "failed";
  });
  return missing.length
    ? Promise.all(missing.map(train => ensureTripContext(S, train)))
    : null;
}
...
export function delayTickerEntryText(S, train, minutes) {
  ...
  /* Lectura operacional inmediata: turno, UT, circulación y retraso. */
  return `${turn} ${train.unit} ${train.circulation} +${minutes}`;
}
...
function fingerprint(train, turn = "") {
  return [
    train.id,
    train.unit,
    train.circulation,
...
export function updateOperationalTrainRow(model, train, S) {
  ...
  const displayedTurn = displayTurnFor(
    S.turnAssignments,
    train.circulation,
    effectiveServiceReference(S, train.id)
  );
  model.row.dataset.turn = displayedTurn;

  const fp = fingerprint(train, displayedTurn);
  ...
  if (model.fingerprint !== fp) {
    ...
    model.turn.textContent = displayedTurn;
