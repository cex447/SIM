...
import { resolveGtfsTimestamp } from './time.js?v=3.39.0';
import { parentCode } from './operations.js?v=3.39.0';
import { cachedPlatform, cachedPlatformByCirculation, normalizePlatformValue } from './isic.js?v=3.39.0';
...
async function loadRouteCatalog() {
  if (!routesPromise) routesPromise = loadJson('data/ctc-routes.json?v=3.39.0', 'ctc-routes.json');
  return routesPromise;
}

async function loadMotionGeometry() {
  if (!motionPromise) {
    motionPromise = loadJson('data/ctc-motion.json?v=3.39.0', 'ctc-motion.json')
      .then(value => {
...
async function loadStationHitGeometry() {
  if (!stationHitPromise) {
    stationHitPromise = loadJson('data/ctc-stations.json?v=3.39.0', 'ctc-stations.json')
      .then(value => {
