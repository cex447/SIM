...
} from "./isic-view-3.42.0.js";

import { updateOccupancy } from "./occupancy-3.42.0.js";
import { BackgroundAudio } from "./audio-3.42.0.js";
import {
  displayTurnFor,
...
} from "./service-calendar-3.42.0.js";

import { setupViewports, activateViewport } from "./viewport-3.42.0.js";
import {
...
const APP_MODULE_VERSION = "3.42.0";
...
async function loadStaticData() {
  const configResponse = await fetch("data/config-3.42.0.json", { cache: "no-store" });
  ...
  const turnsUrl =
    S.config.turnAssignmentsUrl || "data/sim-turnos-servicios-3.42.0.json";
  const [networkResponse, turnsResult] = await Promise.all([
    fetch("data/network-3.42.0.json", { cache: "no-store" }),
