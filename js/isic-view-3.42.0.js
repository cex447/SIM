...
import { decodeCirculation } from "./fgc-api-3.42.0.js";
import {
  getStationCatalog,
  getStationDepartures,
  getTripBundle
} from "./gtfs-3.42.0.js";
import { updateOccupancy } from "./occupancy-3.42.0.js";
import { countdownState, formatOperationalCountdown, resolveGtfsTimestamp } from "./time-3.42.0.js";
import { locateOperationalTarget, parentCode, countdownRedThreshold } from "./operations-3.42.0.js";
import {
  fetchIsicStation,
  fixedPlatformFor,
  fallbackPlatformFor,
...
const FAMILY_ORDER = Object.freeze(["D", "F", "A", "B", "L"]);
const LINE_BY_FAMILY = Object.freeze({ D:"S1", F:"S2", A:"L6", B:"L7", L:"L12" });
const MAX_ROWS_PER_LINE_DIRECTION = 4;

const $ = selector => document.querySelector(selector);
let onSelectTrainHandler = null;
let catalogPromise = null;
let selectedQuickStation = "";
...
function syncQuickStationVisuals() {
  document.querySelectorAll("[data-isic-station]").forEach(button => {
    const selected = Boolean(selectedQuickStation) &&
      button.dataset.isicStation === selectedQuickStation;
...
function clearQuickStationSelection() {
  selectedQuickStation = "";
  syncQuickStationVisuals();
}
...
/* BETA 3.34.0 · una circulación sólo puede existir una vez en iSIC.
...
function mergeDuplicateCirculation(a, b) {
  ...
  return {
    ...
    serviceId:preferred.serviceId || chronological.serviceId || "",
    previousStop:preferred.previousStop || chronological.previousStop || null,
    nextStop:preferred.nextStop || chronological.nextStop || null,
    tripStops:preferred.tripStops || chronological.tripStops || [],
...
function buildScheduleRecord(S, departure) {
  ...
  return {
    ...
    previousStop:departure.previousStop?.code || "",
    nextStop:departure.nextStop?.code || "",
    tripStops:departure.tripStops || [],
    live,
    source:"schedule"
  };
}
...
async function buildLiveStationRecord(S, live, station, nowMs) {
  ...
  try {
    ...
    return {
      ...
      serviceId:bundle.trip?.service_id || "",
      previousStop:(() => {
        const index = times.findIndex(stop => parentCode(stop) === station);
        return index > 0 ? parentCode(times[index - 1]) : "";
      })(),
      nextStop:(() => {
        const index = times.findIndex(stop => parentCode(stop) === station);
        return index >= 0 && index < times.length - 1 ? parentCode(times[index + 1]) : "";
      })(),
      tripStops:times.reduce((codes, stop) => {
...
export function routeSegmentForItem(item) {
  const live = item.live;
  const stationed = Boolean(live?.stationed);

  /* Una fila horaria sin snapshot vivo representa el tramo contiguo a la
     estación consultada: salida hacia la siguiente parada o, si es terminal,
     aproximación desde la anterior. Nunca mostramos "PC → —". */
  let current = item.nextStop ? (item.station || "") : (item.previousStop || "");
  let next = item.nextStop || (item.previousStop ? (item.station || "") : "");

  if (live && !stationed) {
    next = String(live.nextStop || item.nextStop || "").toUpperCase();
    const stops = item.tripStops || [];
    const nextIndex = stops.findIndex(code => code === next);
    current = nextIndex > 0
      ? stops[nextIndex - 1]
      : (next === item.station ? (item.previousStop || "") : "");
  }

  return {
    stationed,
    current:stationed ? "" : current,
    next:stationed ? (live.stationed || item.station || "—") : (next || "—"),
    animated:Boolean(live && !stationed)
  };
}

function routeCells(row, item) {
  const route = routeSegmentForItem(item);

  row.current.textContent = route.current;
  row.current.classList.toggle("state-spacer", route.stationed || !route.current);
  row.arrow.textContent = "→";
  row.arrow.className = route.stationed
    ? "route-arrow route-arrow-placeholder"
    : `route-arrow${route.animated ? " moving" : ""}`;

  row.destination.replaceChildren();
  const code = make(
    "strong",
    `station-token destination-code${route.stationed ? " stationed" : ""}`,
    route.next
  );
...
function createRow(S, item) {
  ...
  button.append(
    row.unit, row.circulation, row.turn, row.occupancy, row.current,
  ...
  row.turn.textContent = displayTurnFor(
    S.turnAssignments,
    item.circulation,
    effectiveServiceReference(S, item.serviceId || item.id)
  );
...
export function renderISIC(S) {
  ...
  /*
   * BETA 3.34.0 · el estado se decide por sentido.
...
export async function syncISICQuery(S, { blur = false } = {}) {
  ...
  if (!code && selectedQuickStation) {
    renderISIC(S);
    return;
  }

  /* Una estación escrita manualmente no mantiene ningún acceso rápido como
     seleccionado. */
  if (code) clearQuickStationSelection();
...
export function wireISIC(S, { onSelectTrain } = {}) {
  ...
  const handle = () => {
    clearQuickStationSelection();
    syncISICQuery(S, { blur:normalizeStationInput(input.value).length === 2 });
  };

  input.addEventListener("pointerdown", () => {
    if (!input.value && !selectedQuickStation && !S.isicView.station) return;
    input.value = "";
    clearQuickStationSelection();
    clearStationQueryState(S, "");
  });
  ...
  document.querySelectorAll("[data-isic-station]").forEach(button => {
    ...
    button.addEventListener("click", async () => {
      const code = String(button.dataset.isicStation || "").toUpperCase();
      selectedQuickStation = code;
      input.value = "";
...
export async function refreshISIC(S, { force = false } = {}) {
  ...
  state.refreshPromise = (async () => {
    ...
    /* BETA 3.34.0: el límite es por línea + sentido, no por sentido global.
       Se conservan como máximo las cuatro circulaciones cronológicamente más
       próximas de cada combinación (S1/S2/L6/L7/L12 × asc/desc). */
