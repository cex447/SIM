...
export function rawTurnFor(assignments, circulation, serviceReference) {
  ...
  if (!CIRCULATION_RE.test(code)) return null;

  const directReference = String(serviceReference || "").trim().toLowerCase();
  /* Les 8xx ordinàries continuen excloses, però un quadre especial datat és
     explícit i pot incorporar circulacions creades només per aquell servei. */
  if (code[1] === "8" && !directReference.startsWith("special:")) return null;
