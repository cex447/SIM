...
} from "./isic-view-3.41.0.js";

import { updateOccupancy } from "./occupancy-3.41.0.js";
import { BackgroundAudio } from "./audio-3.41.0.js";
import {
  displayTurnFor,
...
import { setupViewports, activateViewport } from "./viewport-3.41.0.js";
import {
...
const APP_MODULE_VERSION = "3.41.0";
...
async function loadStaticData() {
  const configResponse = await fetch("data/config-3.41.0.json", { cache: "no-store" });
  ...
  const turnsUrl =
    S.config.turnAssignmentsUrl || "data/sim-turnos-servicios-3.41.0.json";
  const [networkResponse, turnsResult] = await Promise.all([
    fetch("data/network-3.41.0.json", { cache: "no-store" }),
...
function setupDiagnostics() {
  ...
  const open = () => {
    ...
    $("#diagText").textContent = [
      ...
      `Servei BV: ${serviceTypeFromReference(S.query.train?.id || S.trains[0]?.id) || "—"}`,
