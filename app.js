import {fetchPositioning,normalizeTrain} from "./fgc-api.js?v=1.2.0";
import {renderPUV} from "./puv.js?v=1.2.0";
import {loadLIT,tickLIT} from "./lit.js?v=1.2.0";

const S={config:null,interstations:null,trains:[],lastFetch:null,lastError:null,rawCount:0,bvCount:0,activeView:"puv",selected:null};
const $=s=>document.querySelector(s);

async function init(){
  S.config=await fetch("data/config.json?v=1.2.0",{cache:"no-store"}).then(r=>r.json());
  S.interstations=await fetch("data/interstations.json?v=1.2.0",{cache:"no-store"}).then(r=>r.json());
  clock();tabs();filters();diag();
...
async function refresh(){
  try{
    const got=await fetchPositioning(S.config.positioningUrl+"&_ts="+Date.now());
...
function diag(){
  ...
  const start=()=>timer=setTimeout(openDiag,1000);
